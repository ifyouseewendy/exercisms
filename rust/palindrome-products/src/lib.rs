...
impl Palindrome {
    ...
    pub fn from(factors: Vec<(u64, u64)>) -> Palindrome {
        let (a, b) = factors[0];
        Palindrome {
            factors,
            value: a * b,
...
pub fn palindrome_products(min: u64, max: u64) -> Option<(Palindrome, Palindrome)> {
    ...
    Some((
        Palindrome::from(find_factors(min_p, min, max)),
        Palindrome::from(find_factors(max_p, min, max)),
    ))
}
...
fn find_factors(num: u64, min: u64, max: u64) -> Vec<(u64, u64)> {
    let mut factors = vec![];
    ...
    if factors.is_empty() {
        panic!("factors is empty for {}, {}, {}", num, min, max)
    }
